...
void main()
{
	...
	// Light properties
	vec3 LightPosition = vec3(0.0, 400.0, -500.0);
	vec3 LightColor = vec3(1, 1, 1);

	// Material properties
	vec3 MaterialDiffuseColor = vec3(0.0, 0.6, 0.0);
	...
	// Material properties
	vec3 MaterialDiffuseColor2 = vec3(0.0, 0.6, 0.0);
	vec3 MaterialAmbientColor2 = vec3(0.3, 3.0, 0.3) * MaterialDiffuseColor2;
	vec3 MaterialSpecularColor2 = vec3(0.0, 0.0, 0.0);

	float distance = length(LightPosition2 - position);

	vec3 N2 = normalize(normal);
	vec3 L2 = normalize(LightPosition2 - vec3(uv.x, uv.y, position.z));
	...
	if (lambertian2 > 0.0) {
		...
		vec3 V2 = normalize(-vec3(uv.x, uv.y, position.z));
